...
class CreadorViajes:
    ...
    def __recorrido_ciclo_euleriano(self, origen):
        grafo_editable = {}  # Dict de listas
        for v in self.__grafo:
            grafo_editable[v] = []
            for w in self.__grafo.adyacentes(v):
                grafo_editable[v].append(w)
        ...
        while camino_actual:
            v = camino_actual[-1]
            if grafo_editable[v] and not (v, grafo_editable[v][-1]) in aristas_visitadas:
                w = grafo_editable[v].pop()
            ...
            elif grafo_editable[v] and (v, grafo_editable[v][-1]) in aristas_visitadas:
                grafo_editable[v].pop()
            else:
